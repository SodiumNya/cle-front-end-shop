        ...
        <template #label>
          ...
          >
            ...
            <template #value>
              ...
              <van-button v-if="order.statue === 4 && goods.reviewStatus === 0"
                  type="primary" size="small"
              @click="router.push({
              path: api.rate.path,
              query: {goodsId: goods.goodId, orderSn: orderSn}})">
                去评价
              </van-button>
            </template>
...
const fetchData = async (orderSn)=>{
  const detailOrderResp = await request.get(`order/detail/${orderSn}`)
